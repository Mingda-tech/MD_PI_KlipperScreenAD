...
class KlipperScreen(Gtk.Window):
    ...
    def state_paused(self):
        if self._ai_pause_signal_active():
            self._schedule_ai_detection_status_request(reset=True)
        preserve_dialogs = [self.ai_detection_dialog] if self.ai_detection_dialog else None
        self._show_job_status_panel(preserve_dialogs=preserve_dialogs)
        if (
            self.ai_detection_dialog is None
            and self._config.get_main_config().getboolean("auto_open_extrude", fallback=True)
        ):
            self.show_panel("extrude", _("Extrude"))

    def _show_job_status_panel(self, preserve_dialogs=None):
        self.close_screensaver()
        preserve_dialogs = [dialog for dialog in (preserve_dialogs or []) if dialog is not None]
        for dialog in list(self.dialogs):
            if dialog in preserve_dialogs:
    ...
    def _show_ai_detection_dialog(self, result):
        defect_type = canonical_defect_type(result.get("defectType"))
        defect_name = _(
            AI_DETECTION_DISPLAY_NAMES.get(defect_type, defect_type or "Unknown")
        )
        ...
        label.set_markup(
            "<big><b>{}</b></big>\n{}: {}".format(
                GLib.markup_escape_text(_("Defect detected: %s") % defect_name),
        ...
        pixbuf = self._load_ai_detection_pixbuf(result.get("evidencePath"))
